fastboot %* getvar product 2>&1 | findstr /r "vayu bhima" || echo Missmatching image and device
fastboot %* getvar product 2>&1 | findstr /r "vayu bhima" || exit /B 1

::check anti_version
if exist %~dp0images\anti_version.txt (for /f "delims==" %%a in (%~dp0images\anti_version.txt) do (set CURRENT_ANTI_VER=%%a))
if [%CURRENT_ANTI_VER%] EQU [] set CURRENT_ANTI_VER=2
